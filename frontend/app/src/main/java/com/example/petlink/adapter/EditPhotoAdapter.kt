...
class LocalPhotoAdapter(
    ...
    fun submit(list: List<Uri>) { uris.clear(); uris.addAll(list); notifyDataSetChanged() }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_edit_photo, parent, false)
        return VH(v)
    }
    override fun getItemCount(): Int = uris.size
    override fun onBindViewHolder(holder: VH, position: Int) { holder.bind(uris[position], onRemove, position) }
...
class ExistingPhotoAdapter(
    ...
    fun submit(list: List<AnimalPhotoReq>) { photos.clear(); photos.addAll(list); notifyDataSetChanged() }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_edit_photo, parent, false)
        return VH(v)
    }
    override fun getItemCount(): Int = photos.size
    override fun onBindViewHolder(holder: VH, position: Int) { holder.bind(photos[position], onRemove) }
    class VH(v: View): RecyclerView.ViewHolder(v) {
        private val iv: ImageView = v.findViewById(R.id.ivPhoto)
        private val btn: Button = v.findViewById(R.id.btnRemove)
        fun bind(photo: AnimalPhotoReq, onRemove: (AnimalPhotoReq) -> Unit) {
